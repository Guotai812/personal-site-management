...
export async function editIntrodution(id, formData) {
    const db = await getDatabase();
    await db.collection("introduction").updateOne(
        {_id: new ObjectId(id)},
        {$set: {content: formData.get("content")}}
    );
    redirect('/dashboard/introduction');
}
